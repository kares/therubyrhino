  ...
  class Context
    ...
    def run(context) # :nodoc:
      ...
      if @options[:java]
        ...
        @global = JS::ScriptRuntime.initSafeStandardObjects(@native, nil, @options[:sealed] == true)
      end
    ...
    def timeout_limit=(limit)
      if restrictable?
        ...
        raise "setting a timeout_limit has no effect on this context, use " \
              "Context.open(restrictable: true) to gain a restrictable instance"
      end
    end
    ...
    private_constant :CODE_GENERATION_ERROR_MESSAGE
    ...
    private_constant :CODE_GENERATION_TRACE_CLASS_NAME
    private_constant :CODE_GENERATION_TRACE_METHOD_NAME
